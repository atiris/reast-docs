...
export const FEATURES: Feature[] = [
  ...
  },
  {
    ...
    since: '1.0',
    note: {
      en: 'Available since the first release and in real use, but still in its testing phase — assignment and the domain-prefix rule may be refined before they are frozen.',
      sk: 'Dostupné od prvého vydania a v reálnom používaní, ale stále v testovacej fáze — priradenie aj pravidlo prefixu domény sa môžu pred zmrazením upraviť.',
    },
  ...
  },
  {
    ...
    title: {
      en: 'Variable scopes',
      sk: 'Rozsahy platnosti premenných',
    },
    group: 'commands',
    ...
    since: '1.0',
    note: {
      en: 'Story, shared and heading scoping all work; the exact lifetime of a heading-scoped variable across parts is still being validated.',
      sk: 'Rozsah príbehu, zdieľaný aj nadpisový fungujú; presná životnosť premennej v rozsahu nadpisu naprieč časťami sa ešte overuje.',
    },
  ...
  },
  {
    ...
    title: {
      en: 'Built-in namespaces',
      sk: 'Vstavané menné priestory',
    },
    group: 'commands',
    ...
    since: '1.0',
    note: {
      en: 'The namespaces exist and are read-only; how many members each exposes depends on the host and is still growing.',
      sk: 'Menné priestory existujú a sú len na čítanie; koľko členov každý sprístupní, závisí od hostiteľa a stále rastie.',
    },
  ...
  },
  {
    ...
    status: 'draft',
    note: {
      en: 'Discussed and specified down to the union and difference operators, but nothing is built: the expression grammar does not recognise `@` yet, so points and areas exist only on paper. The separator is a semicolon, because a comma already separates the arguments a coordinate sits among.',
      sk: 'Prediskutované a špecifikované až po operátory zjednotenia a rozdielu, ale nič nie je postavené: gramatika výrazov zatiaľ nepozná `@`, takže body a oblasti existujú len na papieri. Oddeľovačom je bodkočiarka, pretože čiarka už oddeľuje argumenty, medzi ktorými súradnica stojí.',
    },
  ...
  },
  {
    ...
    status: 'draft',
    note: {
      en: 'Testing the reader’s position against an area in an ordinary expression. Blocked on coordinate literals, which are themselves a draft — today only `{waypoint}` understands coordinates, and it parses them itself.',
      sk: 'Testovanie polohy čitateľa proti oblasti v bežnom výraze. Blokované literálmi súradníc, ktoré sú samy v návrhu — dnes súradniciam rozumie len `{waypoint}` a parsuje si ich sám.',
    },
  ...
  },
  {
    ...
    status: 'development',
    note: {
      en: 'Geocaching-style named places gated on arrival. The command parses into structured coordinates and the distance maths is done; the reader-side gate and permission flow are being built.',
      sk: 'Pomenované miesta v štýle geocachingu podmienené príchodom. Príkaz sa parsuje na štruktúrované súradnice a výpočet vzdialenosti je hotový; brána na strane čitateľa a tok povolení sa stavajú.',
    },
  ...
  },
  {
    ...
    status: 'development',
    note: {
      en: 'An author-supplied image anchored to real GPS bounds, with pins at fixed coordinates or variable references. Parsed into structured nodes; the projection renderer in the reader is the remaining piece.',
      sk: 'Obrázok dodaný autorom ukotvený na reálne GPS hranice, so špendlíkmi na pevných súradniciach alebo odkazoch na premenné. Parsuje sa na štruktúrované uzly; zostávajúcim dielom je renderer projekcie v čítačke.',
    },
  ...
  },
  {
    ...
    status: 'draft',
    note: {
      en: 'Chaining waypoints into an ordered or free-order trail. Specified; waiting on waypoints to land first.',
      sk: 'Reťazenie zastávok do usporiadaného alebo voľného chodníka. Špecifikované; čaká, kým dorazia zastávky.',
    },
  ...
  },
  {
    ...
    status: 'draft',
    note: {
      en: 'Enter and exit events for an area. Specified, but it depends on both coordinate literals and continuous background positioning, neither of which exists.',
      sk: 'Udalosti vstupu a odchodu pre oblasť. Špecifikované, ale závisí to od literálov súradníc aj od nepretržitého určovania polohy na pozadí, pričom ani jedno neexistuje.',
    },
  ...
  // ── Authoring & diagnostics ───────────────────────────────────────────────
  {
    ...
    since: '1.0',
    note: {
      en: 'Both forms are frozen; `//` makes the parser ignore everything inside the braces, `begin` included.',
      sk: 'Obe formy sú zmrazené; `//` spôsobí, že parser ignoruje všetko vnútri zátvoriek vrátane `begin`.',
    },
  ...
  },
  {
    ...
    since: '1.0',
    note: {
      en: 'A reader never sees an error: unknown commands print, missing media falls back, unclosed blocks auto-close. This is a language guarantee, not a runtime detail, so it is frozen.',
      sk: 'Čitateľ nikdy neuvidí chybu: neznáme príkazy sa vytlačia, chýbajúce médiá majú náhradu, neuzavreté bloky sa uzavrú samy. Toto je záruka jazyka, nie detail runtime, takže je zmrazená.',
    },
  ...
  },
  {
    ...
    status: 'draft',
    note: {
      en: 'Author-visible inline warnings with typo suggestions. The validator already reports most of these outside the story; the in-story `{strict on}` switch is specified only.',
      sk: 'Inline varovania viditeľné pre autora s návrhmi pri preklepoch. Validátor väčšinu z nich už hlási mimo príbehu; prepínač `{strict on}` priamo v príbehu je len špecifikovaný.',
    },
  ...
  },
  {
    id: 'todo-note',
    title: {
      en: 'TODO & note markers',
      sk: 'Značky TODO a poznámok',
    },
    group: 'authoring',
    syntax: '{todo: …} · {note: …}',
    status: 'draft',
    note: {
      en: 'Development annotations that surface as build warnings. Specified; today a `{// comment}` does the same job silently.',
      sk: 'Vývojárske anotácie, ktoré sa objavia ako varovania pri zostavení. Špecifikované; dnes tú istú prácu ticho zastane `{// komentár}`.',
    },
