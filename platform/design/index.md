...
- **`.rext` extensions are sandboxed Rea, not arbitrary code.** They cannot access the filesystem, the network beyond what the engine's built-ins allow, or any API outside the Rea language — see [Sandbox constraints](/spec/05-reference#_31-extensibility). A story cannot embed JavaScript or any other language.
